...
std::string files[_TOTAL_IMAGES] = {"/images/image1.png",
                                    "/images/image2.png",
                                    "/images/image3.png",};


std::vector < HeightField > heightfield;
...
void init(){
  ...
  for(unsigned int i=0; i < _TOTAL_IMAGES; i++){
    ...
    unsigned int normals_bytes  = heightfield[i].normals.size()*sizeof(vec3);
...
int main(void){
  ...
  while (!glfwWindowShouldClose(window)){
    ...
    glUniformMatrix4fv( ModelView_loc, 1, GL_TRUE, user_MV*heightfield[current_draw].model_view);
    glUniformMatrix4fv( Projection_loc, 1, GL_TRUE, projection );
    glUniformMatrix4fv( NormalMatrix_loc, 1, GL_TRUE, transpose(invert(user_MV*heightfield[current_draw].model_view)));

    glDrawArrays( GL_TRIANGLES, 0, heightfield[current_draw].vertices.size() );
    // ====== End: Draw ======
